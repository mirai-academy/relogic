    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-500 relative overflow-hidden ${isDark ? 'bg-cyber-black text-white' : 'bg-stone-50 text-stone-900'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center relative z-10">
        <motion.div
          ...
          className="space-y-8"
        >
          <div className={`w-32 h-32 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-2xl mb-10 border-2 ${isDark ? 'bg-neon-cyan/10 border-neon-cyan/30 text-neon-cyan cyber-glow' : 'bg-blue-500 text-white'}`}>
            <ShoppingBag className="w-16 h-16 drop-shadow-[0_0_10px_currentColor]" />
          </div>
          ...
          <p className={`font-bold font-tech tracking-tight ${isDark ? 'text-neon-cyan/60' : 'text-stone-500'}`}>{t.stay_tuned}</p>
